...
# ---------------------------------------------- ADR-0002: glossary mirror

def test_glossary_mirror_written_on_ingest(wiki: Path):
    _seed_raw(wiki, "gm1")
    ingest_one(wiki, "gm1", _GlossaryAgent([("Mirrorable", "first def")]))
    mirror = wiki / "wiki" / "concepts" / "_glossary.md"
    assert mirror.is_file(), "_glossary.md must be regenerated on ingest"
    text = mirror.read_text(encoding="utf-8")
    assert "generated_by: glossary-mirror" in text
    assert "## Mirrorable" in text
    assert "first def" in text
    assert "[SCHEMA.md](../../SCHEMA.md#glossary)" in text


def test_glossary_mirror_alphabetical_and_regenerates(wiki: Path):
    _seed_raw(wiki, "gm2")
    ingest_one(wiki, "gm2", _GlossaryAgent([("Zeta", "z def"), ("alpha", "a def")]))
    text = (wiki / "wiki" / "concepts" / "_glossary.md").read_text(encoding="utf-8")
    a = text.find("## alpha")
    z = text.find("## Zeta")
    assert 0 < a < z, "mirror must be alphabetical case-insensitive"

    # Second ingest adds a new term: mirror is regenerated to include it.
    _seed_raw(wiki, "gm3", body="x\n")
    ingest_one(wiki, "gm3", _GlossaryAgent([("Beta", "b def")]))
    text2 = (wiki / "wiki" / "concepts" / "_glossary.md").read_text(encoding="utf-8")
    assert "## alpha" in text2
    assert "## Beta" in text2
    assert "## Zeta" in text2
