...
def integration_env(tmp_path):
    ...
    # 1. Run Init (M1)
    subprocess.run([
        sys.executable, str(project_root / "src/wiki/init.py"),
        "init", "IntegrationTest", "Test wiki for M14", str(wiki_root)
    ], check=True)
...
def test_end_to_end_pipeline(integration_env):
    ...
    env = {**os.environ, "LLMWIKI_TEST_STUB_AGENT": "1"}
    subprocess.run([
        sys.executable, "-m", "wiki.ingest", "alpha-source", "--wiki-root", str(wiki_root)
    ...
    # Stub embedder produces hash-based scores; lower thresholds so retrieval succeeds
    cfg_data["retrieval"]["min_score"] = 0.0
    cfg_data["retrieval"]["ood_threshold"] = 0.0
    
    with open(test_config_path, 'w') as f:
        yaml.dump(cfg_data, f)
        
    env_rag = {**os.environ, "LLM_RAG_WIKI_TEST_STUB_EMBEDDER": "1"}
    subprocess.run([
        sys.executable, "-m", "rag.ingest", "--config", str(test_config_path)
    ...
    result = subprocess.run([
        sys.executable, "-m", "rag.retrieve", "Alpha", "--config", str(test_config_path), "--format", "json"
