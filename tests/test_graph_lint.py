...
def test_orphan_and_index_gap_exempt_glossary_mirror(wiki: Path):
    """Per ADR-0002: ``_glossary.md`` is automation-owned, exempt from
    ``orphan`` and ``index_gap``. ``broken_link`` still applies."""
    _write(wiki / "wiki/concepts/_glossary.md", _page("_glossary"))
    r = lint_wiki(wiki)
    assert not any(
        i["rule"] == "orphan" and i.get("page", "").endswith("_glossary.md")
        for i in r.issues
    )
    assert not any(
        i["rule"] == "index_gap" and i.get("page", "").endswith("_glossary.md")
        for i in r.issues
    )
