...
def _make_cfg(tmp_path: Path) -> Config:
    ...
    return Config(
        ...
        snapshot=SnapshotConfig(enabled=False, backup_dir=(tmp_path / "snap").resolve()),
...
def test_ingest_edited_file_replaces_chunks(tmp_path: Path) -> None:
    ...
    assert m2.files["concepts/alpha.md"].chunk_ids != tuple(old_alpha_ids)
    assert list(m2.files["concepts/beta.md"].chunk_ids) == untouched_ids
