...
See MASTER.md Appendix F (rebuild registry, M12).
"""
...
class PersonaCompiler:
    """Implements token-optimized compilation per MASTER Appendix F (M12)."""
    ...
    def compile_dense(self, personas: List[Persona], meta_directives: List[MetaDirective]) -> str:
        ...
        Determinism (MASTER §6 MCP invariant 2): the output depends only on
        the *set* of inputs, never on input list order. Personas are sorted
        by ``(kind, id)`` before any iteration.
        """
        # Precedence: Meta-Directives -> Personas
        personas = sorted(personas, key=lambda p: (p.kind, p.id))
        ...
        full_content = f"{meta_ids}|{','.join(chars)}|{','.join(doms)}|{';'.join(all_rules)}"
...
def compile_profile(personas: List[Persona], meta_directives: List[MetaDirective]) -> str:
    """Module-level entry point used by server.py: compile to dense runtime profile string.

    Raises ``RuntimeError`` tagged ``[ERR_COMPILE]`` if the underlying compiler
    fails to produce a non-empty dense profile (per MASTER §9 MCP Error Classes).
    """
    try:
        out = PersonaCompiler().compile_dense(personas, meta_directives)
    except Exception as e:
        raise RuntimeError(f"[ERR_COMPILE] dense profile compilation failed: {e}") from e
    if not out:
        raise RuntimeError("[ERR_COMPILE] dense profile compilation produced empty output")
    return out
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m persona_mcp.compiler <persona_id_1> [persona_id_2] ...")
        sys.exit(1)
    ...
    try:
        ...
        print(f"[ERR_MCP_RUNTIME] Compilation failed: {e}")
        sys.exit(5)
