...
# Generated pages are owned by automation; they are not authored cross-links
# and are not expected to appear in index.md. Per ADR-0002.
_GENERATED_BASENAMES: frozenset[str] = frozenset({"_glossary.md"})
...
def _lint_rules(
    ...
    # generated pages (ADR-0002) exempt.
    for rel in nodes:
        if (
            inbound.get(rel, 0) == 0
            and "/sources/" not in rel
            and os.path.basename(rel) not in _GENERATED_BASENAMES
        ):
            issues.append({
                "severity": "high",
                "rule": "orphan",
    ...
    # 3. index_gap (MEDIUM) — page not mentioned in index.md; generated
    # pages (ADR-0002) exempt.
    for rel in nodes:
