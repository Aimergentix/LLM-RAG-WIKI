...
write_frontmatter() {
    ...
    local body title_yaml
    body="$(cat "$outfile" 2>/dev/null || true)"
    # Encode the title via PyYAML so newlines, quotes, leading >/&/*, and
    # other YAML-significant characters can never break the frontmatter.
    title_yaml="$(python3 - "$title" <<'PY'
import sys, yaml
title = sys.argv[1]
# Cap absurdly long titles so the frontmatter stays readable.
if len(title) > 240:
    title = title[:237] + "..."
sys.stdout.write(yaml.safe_dump({"title": title}, default_flow_style=False, allow_unicode=True).rstrip("\n"))
PY
)"
    {
        echo "---"
