...
def ingest_one(
    ...
    *,
    ...
    # MASTER §6 W3 step 3: "Extract 3–6 takeaways".
    if not 3 <= len(takeaways) <= 6:
        raise IngestError(
            ERR_RUNTIME,
            f"agent returned {len(takeaways)} takeaways (need 3-6)",
            EXIT_RUNTIME,
        )
    ...
    # Per ADR-0002: regenerate the glossary mirror so the RAG layer can
    # retrieve domain vocabulary that lives in SCHEMA.md (which is outside
    # wiki_root). Always regenerate from the current schema text so the
    # mirror tracks both manual and auto rows.
    mirror_path = wiki_root / "wiki" / "concepts" / "_glossary.md"
    mirror_text = glossary_mod.render_mirror(new_schema, date)
    write_plan[mirror_path] = mirror_text
    ...
    # Cross-layer dirty sentinel — written only when the caller supplies a
    # path via ``rag_sentinel``.  The path must be outside wiki_root (the
    # RAG layer owns it; wiki_root is a read-only upstream per MASTER §8).
    # Default is None so that standalone wiki operation touches nothing
    # outside wiki_root.  The CLI flag ``--rag-sentinel`` enables this.
    if rag_sentinel is not None:
        try:
            rag_sentinel.parent.mkdir(parents=True, exist_ok=True)
            rag_sentinel.touch(exist_ok=True)
            os.utime(rag_sentinel, None)
        except OSError:
            pass
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Optional path for a cross-layer dirty sentinel file. "
            "Must be outside wiki_root (e.g. data/manifests/.wiki_dirty). "
    ...
    try:
        ...
        rag_sentinel = Path(args.rag_sentinel) if args.rag_sentinel else None
        report = ingest_one(wiki_root, slug, agent, force=args.force, rag_sentinel=rag_sentinel)
