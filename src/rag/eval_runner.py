...
def _build_backends(cfg: Config):
    """Build (embedder, adapter) honoring the test-stub env var.

    Mirrors the selection logic in ``rag.ingest._build_embedder`` and
    ``rag.retrieve.main`` so the eval harness runs offline in CI.
    """
    if os.environ.get("LLM_RAG_WIKI_TEST_STUB_EMBEDDER") == "1":
        from rag.embedder import DeterministicHashEmbedder
        from rag._query_store import InMemoryQueryAdapter
        from rag.store import InMemoryVectorStore

        embedder = DeterministicHashEmbedder(dim=16)
        # In-memory adapter requires a populated store; the harness
        # caller is responsible for seeding it. Empty store yields
        # out_of_domain responses, which is acceptable for stub mode.
        adapter = InMemoryQueryAdapter(InMemoryVectorStore())
        return embedder, adapter
...
def run_evals(cfg: Config, cases_path: Path) -> List[EvalResult]:
    ...
    embedder, adapter = _build_backends(cfg)
