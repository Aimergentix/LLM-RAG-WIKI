...
@dataclass(frozen=True)
class OpsConfig:
    """Operational tuning knobs (per-call rendering limits, etc.)."""
...
def _check_outside_wiki_root(resolved: Path, field: str, wiki_root: Path) -> None:
    """Reject write-paths that resolve under wiki_root.

    Build-pipeline artefacts (vector index, manifest, snapshots) must live
    outside wiki_root, or a later ingest pass would treat its own outputs as
    source markdown. Equality is also rejected.
    """
    try:
        ...
        return
    raise ConfigError(
        f"{ERR_CONFIG} {field} must not be under paths.wiki_root"
        f" ({wiki_root}): {resolved}"
    )
...
def _parse(raw: dict, config_dir: Path) -> Config:
    ...
    # MASTER §8: write-path containment. The build pipeline must not write
    # generated artefacts (index, manifest, snapshots) under wiki_root, or a
    # subsequent ingest pass would treat its own outputs as source markdown.
    _check_outside_wiki_root(paths.index_dir, "paths.index_dir", paths.wiki_root)
    _check_outside_wiki_root(
        paths.manifest_path, "paths.manifest_path", paths.wiki_root
    )
    ...
    # snapshot
    snap = _section(raw, "snapshot")
    snapshot = SnapshotConfig(
        enabled=_leaf(snap, "enabled", bool),  # type: ignore[arg-type]
        backup_dir=_resolve_path(snap.get("backup_dir"), "snapshot.backup_dir", config_dir),
    )
    _check_outside_wiki_root(
        snapshot.backup_dir, "snapshot.backup_dir", paths.wiki_root
    )
    ...
    _strategy: str = chunking.strategy
    if _strategy not in {"heading_aware"}:
        raise ConfigError(
            f"{ERR_CONFIG} chunking.strategy must be 'heading_aware',"
            f" got {_strategy!r}"
        )
    ...
    if top_k < 1:
        raise ConfigError(
            f"{ERR_CONFIG} retrieval.top_k must be >= 1, got {top_k}"
        )
    ...
    if retrieval.distance_metric != "cosine":
        # Score conversion in `_query_store.ChromaQueryAdapter.query` and the
        # MMR similarity in `retrieve._apply_mmr` both assume cosine
        # similarity in [-1, 1]. Restrict the metric until those callers
        # gain dedicated branches for ip / l2.
        raise ConfigError(
            f"{ERR_CONFIG} retrieval.distance_metric must be 'cosine',"
            f" got {retrieval.distance_metric!r}"
        )
    ...
    # ops (optional section; all keys have defaults)
    ops_raw = raw.get("ops") or {}
    if not isinstance(ops_raw, dict):
        raise ConfigError(
            f"{ERR_CONFIG} section 'ops' must be a mapping,"
            f" got {type(ops_raw).__name__}"
        )
    if "max_context_chars" in ops_raw:
        mcc = _leaf(ops_raw, "max_context_chars", int)  # type: ignore[assignment]
        if mcc < 1:
            raise ConfigError(
                f"{ERR_CONFIG} ops.max_context_chars must be >= 1, got {mcc}"
            )
        ops = OpsConfig(max_context_chars=mcc)  # type: ignore[arg-type]
    else:
        ops = OpsConfig()
