...
class QueryableStore(Protocol):
    def query(self, embedding: list[float], top_k: int) -> list[QueryHit]:
        """Query the store for the top-k nearest neighbors by embedding vector."""
        ...


@runtime_checkable
class LexicalQueryable(Protocol):
    def query(self, text: str, top_k: int) -> list[QueryHit]:
        """Query the store for the top-k results by raw text (e.g. BM25)."""
        ...
...
class BM25QueryAdapter(LexicalQueryable):
