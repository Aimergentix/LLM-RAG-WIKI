    ...
    *,
    ...
    try:
        for fpath in files:
            ...
            # Design intent: injection-flagged chunks are indexed here but withheld
            # at retrieval time by _format_excerpt (defense-in-depth). Blocking at
            # ingest would prevent the chunk from being discovered at all, which may
            # suppress legitimate content that happens to share a marker substring.
            # To block at ingest instead, move this check before the upsert call.
            for c in chunks:
            ...
            stats_indexed += 1
            new_files[rel_path] = FileEntry(source_hash=source_hash, chunk_ids=tuple(ids))
    ...
    if use_atomic:
        if own_store:
            ...
            if isinstance(write_store, ChromaVectorStore) and isinstance(
                ...
                # Two-phase rename so a failure in the swap leaves the
                # previous primary intact:
                #   1. Rename live primary aside  (canonical -> __old).
                #   2. Rename shadow into canonical  (__shadow -> canonical).
                #   3. Drop the now-orphaned aside.
                # If step 2 fails we restore the aside back to canonical.
                aside_name = collection_name + "__old"
                stashed = False
                try:
                    primary_store._collection.modify(name=aside_name)  # noqa: SLF001
                    stashed = True
                    write_store._collection.modify(name=collection_name)  # noqa: SLF001
                    try:
                        primary_store._client.delete_collection(aside_name)  # noqa: SLF001
                    except Exception:  # noqa: BLE001 — aside cleanup is best-effort
                        LOG.warning(
                            "rag.ingest atomic reindex: failed to drop aside %s",
                            aside_name,
                        )
                except Exception as exc:  # noqa: BLE001
                    LOG.error(
                        "rag.ingest atomic reindex swap failed: %s", exc
                    )
                    if stashed:
                        # Roll back: restore primary from aside.
                        try:
                            primary_store._collection.modify(  # noqa: SLF001
                                name=collection_name
                            )
                        except Exception:  # noqa: BLE001
                            LOG.error(
                                "rag.ingest atomic reindex rollback also failed;"
                                " manual recovery required (aside=%s, shadow=%s)",
                                aside_name,
                                _shadow_collection_name(collection_name),
                            )
                    raise
    ...
    # Clear the cross-layer dirty sentinel written by wiki.ingest.
    # Sentinel lives under manifest_path.parent (RAG-owned) — not inside
    # wiki_root, which is a read-only upstream per MASTER §8.
    # Best-effort — absence is the steady state.
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # MASTER §9: [ERR_CONFIG] -> exit 2.
        print(str(exc), file=sys.stderr)
        return 2
    except ManifestError as exc:
        # MASTER §9: [ERR_INDEX_MISSING] -> exit 3, [ERR_SCHEMA] -> exit 2.
        msg = str(exc)
