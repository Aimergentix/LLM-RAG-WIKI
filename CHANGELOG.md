...
## [2.1.0-alpha] — 2026-05-02
### Added
- **K0 — Spark surface boundary contract.** `adr/ADR-0003-spark-surface-boundary.md` accepted:
  localhost-only bridge bind (`127.0.0.1`), CORS locked to `spark.github.com`, GET-only
  retrieval routes, data locality invariant (wiki pages, ChromaDB index, manifests, and
  persona files never leave localhost), bearer-token auth via `SPARK_BRIDGE_TOKEN` env var,
  and formal relaxation protocol requiring a superseding ADR.
- **K-Track phase definitions.** `MASTER.md §6` extended with K0–K3 phase contracts
  (Spark surface track): architecture diagram, per-phase deliverables, acceptance criteria,
  and hard constraint summary referencing ADR-0003.
- **`spark-builder` chatmode.** `.github/chatmodes/spark-builder.chatmode.md` added as the
  GitHub Copilot entry point for K-track work. Contains the full system prompt prologue
  covering the Spark SDK, FastAPI bridge, existing read-only pipeline, and phase gate
  discipline. Listed in `MASTER.md §13` Agent Entry Points table.
- **ADR-0003 registered** in `MASTER.md §13` Open ADRs table (status: `accepted`).
...
- **Audit pass.** All 18 findings from the deep-improvement audit landed: bracketed exit codes (FINDING-02/03/16), max_context_chars wired (FINDING-04), write-path containment (FINDING-05), word-boundary injection markers (FINDING-06), deterministic compiler ordering (FINDING-07), HTML-entity glossary escaping (FINDING-08), stale-manifest enforcement with `--allow-stale` flag (FINDING-10), `.rag_dirty` cross-layer sentinel (FINDING-11), `flock`-guarded lint cron (FINDING-12), UTF-8 reads in persona store (FINDING-13), takeaway range 3–6 (FINDING-14), stale-ref cleanup (FINDING-15/18).
- **ADR-0001 accepted (partial).** S0–S5 phase rename applied; MASTER §9 gained `### MCP Error Classes` table; `[ERR_PERSONA_NOT_FOUND]`, `[ERR_PERSONA_SCHEMA]`, `[ERR_COMPILE]`, `[ERR_MCP_RUNTIME]` wired into `src/persona_mcp/`. `[ERR_MCP_BIND]` reserved (moot under stdio transport).
- **ADR-0002 accepted.** Glossary mirror at `wiki/concepts/_glossary.md` regenerated on every W3 ingest from `SCHEMA.md ## Glossary`. Lint exempts `_glossary.md` from `orphan` and `index_gap`.
...
### Added
- **M13 — MCP server.** `src/persona_mcp/server.py` implementation using FastMCP.
...
### Added
- **M12 — Persona compiler.** `src/persona_mcp/compiler.py` implementation.
...
### Added
- **M11 — Persona store.** `src/persona_mcp/store.py` implementation.
